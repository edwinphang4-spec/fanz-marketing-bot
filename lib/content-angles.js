...
const ANGLES = {
  knowledge: {
    ...
    guidance:
      // 2026-08-05:原本这里还写着"可以用官方选尺寸表"。那张表存证失败已停用，
      // 指路指到一个不存在的东西，模型会自己补一个出来 —— 所以指路也一起删掉。
      ...
      'or on this post\'s assigned model. Teach a choice we DO have real data for: light (L) ' +
      'versus no light (N), a wood finish versus matt black in a given room style, or how ' +
      'diameter changes what a fan suits — described in words, never as a room-to-inch rule. ' +
      'We have NO verified room-size-to-fan-size table: never state what size a bedroom, living ' +
      'room or small room "needs". Never invent a number to teach with.',
  },
  scenario: {
    zh: '场景',
...
// ── 分布硬指标 ──
//
// 这几个数原本是按"整月 12 篇"定的。2026-08-08 起批量不再固定:
// 她可能说"下星期三篇"。3 篇时 MIN_DISTINCT_ANGLES=5 **不是不适用,是算不出来**
// (3 篇最多 3 类),MAX_SPEC_POSTS=3 则形同虚设(3 篇可以全是卖点帖)。
//
// 所以改成从批量 n 推。**不做第二套"小批量规则"** —— 两套规则就是两个地方要同步,
// 我们已经栽过一次(提示词说 7 类角度、代码按另外 7 类校验,慢慢漂开)。一处真源。
//
// 更重要的一层:批量越小,"批内多样性"这件事本身越没意义 ——
// 3 篇本来就不会看起来单调,真正的风险是这 3 篇跟上个月发过的重复。
// 那个责任在跨月记忆(content-history)那边。**不是把约束放松了,是责任划分对了。**
const MONTH_BASELINE = 12;           // 这几个数当初就是按 12 篇定的
const MAX_SPEC_POSTS = 3;            // 12 篇时:产品角度最多 3 篇
const MIN_DISTINCT_ANGLES = 5;       // 12 篇时:至少覆盖 5 类角度
const MAX_SAME_ANGLE_PER_PILLAR = 2; // 同一 pillar 内同一角度最多 2 篇(与批量无关)

/**
 * 按批量缩放后的指标。
 *
 * 一律**从上面那三个基准值按比例推**,不另写魔数 —— 否则改了基准值这里不会跟着变,
 * 又是两处真源。硬性验收:n=12 必须原样还原成 5 / 3(见 test-content-angles)。
 *
 * 角度数下限取 2(n≥2 时):3 篇全同一个角度读起来就是三胞胎,
 * 而按比例算 round(5×3/12)=1 会把这条约束整个放掉。
 * 品牌事实用 **floor** 不用 round:宁可少提 —— 3 篇里塞 3 条品牌事实,
 * 正是我们当初要拆掉的"每篇都在喊保修"。
 */
function distributionTargets(n) {
  const count = Math.max(1, Math.round(Number(n) || 0));
  const scale = (base) => Math.round(base * count / MONTH_BASELINE);
  return {
    minDistinctAngles: Math.min(count, Math.max(count >= 2 ? 2 : 1, scale(MIN_DISTINCT_ANGLES))),
    maxSpecPosts: Math.max(1, scale(MAX_SPEC_POSTS)),
    maxSameAnglePerPillar: MAX_SAME_ANGLE_PER_PILLAR,
  };
}

/** 品牌事实配额按批量缩放(向下取整 —— 宁可少提)。 */
function brandFactQuota(key, n) {
  const f = BRAND_FACTS[key];
  if (!f) return 0;
  const count = Math.max(1, Math.round(Number(n) || 0));
  return Math.floor(f.quota * count / MONTH_BASELINE);
}
