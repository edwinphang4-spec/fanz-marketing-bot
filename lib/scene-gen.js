...
async function callGptImage2(prompt, imagePath) {
  ...
  // GPT Image 2 rejects SVG (400 "Invalid image file or mode") and most of
  // the product library is SVG — rasterize to PNG via sharp first.
  if (ext === '.svg') {
    const sharp = require('sharp');
    imageBuffer = await sharp(imageBuffer, { density: 300 })
