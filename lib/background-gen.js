...
const { listReferenceAssets } = require('./brand');
const { buildFullAiPrompt } = require('./design-templates');
...
// 两套：实景类模板要"照片级室内+天花板留位"；图形类模板（product_intro/
// promotion 等深蓝渐变/放射光那种）不能被"interior photograph"锁死——
// 2026-07-24 实测：旧单套约束让图形模板永远出客厅。
...
const HARD_CONSTRAINTS_GRAPHIC =
  'High-end graphic poster background artwork (NOT an interior photograph, NOT a room). ' +
  'STRICT RULES: absolutely NO ceiling fan or any kind of fan. NO text, letters, numbers, ' +
  'watermarks or logos. NO people. NO furniture. Keep generous clean empty space in the ' +
  'upper half (a product image will be placed there later) and keep the lower-left third ' +
  'calm and uncluttered (a title will be placed there later).';

function isDryRun() {
  return !process.env.OPENAI_API_KEY;
}

// ── 参考图视觉风格摘要（OpenRouter gpt-4o vision，10 分钟内存缓存）──
// key: sorted asset-id set joined by ',' → value: { summary, ts }
const _refStyleCache = new Map();
const REF_STYLE_TTL_MS = 10 * 60 * 1000;

/**
 * Call OpenRouter gpt-4o vision to summarise the shared visual style of up to
 * 4 reference-design images. Returns a <=60-word string, or null on any failure.
 * Results are cached 10 minutes keyed by the set of asset IDs (order-independent).
 *
 * @param {Array<{id: string, public_url: string}>} refs
 * @returns {Promise<string|null>}
 */
async function analyseReferenceStyle(refs) {
  if (!refs || refs.length === 0) return null;
  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey) return null;

  const cacheKey = refs.map((r) => r.id).sort().join(',');
  const cached = _refStyleCache.get(cacheKey);
  if (cached && Date.now() - cached.ts < REF_STYLE_TTL_MS) return cached.summary;

  // Build vision content: one image_url entry per reference asset
  const imageContent = refs
    .filter((r) => r.public_url)
    .map((r) => ({ type: 'image_url', image_url: { url: r.public_url, detail: 'low' } }));
  ...
  const content = [
    ...
    {
      type: 'text',
      text: 'Summarise the shared visual style of these past brand designs in <=60 words: colour palette, lighting, composition, mood. Reply with the summary only.',
    },
  ];
  ...
  try {
    ...
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          ...
          'X-Title': 'Fanz Marketing Bot - Reference Style Analysis',
        },
        body: JSON.stringify({
          model: 'openai/gpt-4o',
    ...
    if (summary) _refStyleCache.set(cacheKey, { summary, ts: Date.now() });
    return summary;
  } catch (err) {
    // 静默失败：不影响现有流程
    console.error('[background-gen] reference style analysis skipped:', err.message);
    return null;
  }
}
...
function buildBackgroundPrompt(sceneDescription, brandStyle, referenceStyleSummary, templateStyle, constraintMode) {
  // templateStyle（来自 design-templates 的九模板背景约束）先于品牌全局风格，
  // 决定"这一类帖长什么样"；brandStyle 仍作全局锚点。
  const tpl = templateStyle ? `\n\nTEMPLATE ART DIRECTION: ${templateStyle}.` : '';
  ...
  // referenceStyleSummary（可选）：从 brand_assets kind='reference' 视觉分析提炼，
  // 注入历史设计的共同视觉语言，让生图更贴近品牌已有素材风格。
  const refStyle = referenceStyleSummary
    ? `\n\nSTYLE REFERENCE (derived from past brand designs): ${referenceStyleSummary}`
    : '';
  const hard = constraintMode === 'graphic' ? HARD_CONSTRAINTS_GRAPHIC : HARD_CONSTRAINTS;
  return `${sceneDescription}${tpl}${style}${refStyle}\n\n${hard}`;
}
 ...
 */
async function generateBackground(row, sceneHint, brandStyle, template) {
  let prompt;
  if (template && template.mode === 'full_ai') {
    // festival 类：整图 AI 生成（含节庆排版文字），跳过场景推导与 reference 摘要。
    // 2026-08-06:sceneHint(审核者的意见)以前在这条路上被整个丢掉 —— 节庆图
    // 说"字太大"重出一张,一样是随机结果。现在传下去。
    prompt = buildFullAiPrompt(row, template, sceneHint);
  } else {
    // 图形类模板(sceneSource='template'):模板约束就是场景本身,跳过 LLM 场景
    // 推导——推导出的"客厅实景"会抢走图形模板的主导权(2026-07-24 实测)。
    let description;
    if (template && template.sceneSource === 'template' && template.backgroundStyle && !sceneHint) {
      description = `${template.backgroundStyle}. Theme context: ${(row && row.topic) || 'brand post'}`;
    } else {
      ({ description } = await deriveSceneDescription(row, sceneHint));
    }

    // reference 风格摘要只取同模板 tag 的素材（brand.js 按 series 分桶），
    // 无同 tag 素材则跳过——绝不跨 tag 混风格。静默失败不影响主流程。
    let referenceStyleSummary = null;
    try {
      const refs = await listReferenceAssets(template ? template.tag : undefined);
      if (refs.length > 0) {
        referenceStyleSummary = await analyseReferenceStyle(refs);
      }
    } catch (_) {}

    const tplStyle = (template && template.sceneSource === 'template') ? null : (template ? template.backgroundStyle : null);
    const constraintMode = (template && template.sceneSource === 'template') ? 'graphic' : 'interior';
    prompt = buildBackgroundPrompt(description, brandStyle, referenceStyleSummary, tplStyle, constraintMode);
  }
