 ...
 * 文字区的上沿(占画布高的比例)。
 *
 * 2026-08-03:09-30 那张的标题排到了画布 30% 高的位置,logo 一挪到左上就跟它挤在
 * 一起,而右上角空着。修法本来想「挪位时检测文字在哪、避开」,但三种探测全部失败
 * (OCR 读不出白字压浅底、灰度方差被家具主导、边缘能量分不开窗帘和文字)。
 *
 * 探测不出来就别猜 —— 改成**由我们规定**:提示词要求所有排版文字都在这条线以下,
 * 于是文字区变成已知量,pickLogoPlacement 直接据此排除角落,一次检测都不用做。
 * 副作用是两个顶部角落被保证空出来,logo 怎么挪都不会压到字。
 */
const TEXT_ZONE_TOP = 0.42;

/** 供 pipeline 传给 pickLogoPlacement 的文字区(与提示词里那条线是同一个数) */
const TEXT_ZONE = { top: TEXT_ZONE_TOP, bottom: 1, gap: 0.18 };

// 节庆图(full_ai)的排版文字是模型画的,而且**居中** —— 上下两头都是空的。
// 套用上面那个"文字在下半部"的区,四个角会全被判成压文字(2026-08-06 实测)。
// 这里的上下界对应 buildFullAiPrompt 里要求留白的四角(外侧 20% 高),
// gap 也小得多:居中排版和角落之间本来就有距离,不需要再留 18% 的缓冲。
const FESTIVAL_TEXT_ZONE = { top: 0.22, bottom: 0.78, gap: 0.03 };
...
function typographyBlock(texts, brandBlue) {
  ...
  return (
    ...
    `- PLACEMENT (strict): every piece of typography — headline, subheading and button — must sit ` +
    `entirely BELOW the upper ${Math.round(TEXT_ZONE_TOP * 100)}% of the image height. The top ` +
    `${Math.round(TEXT_ZONE_TOP * 100)}% of the canvas must contain NO text at all, on either side. ` +
    `That band is reserved so the brand logo can be composited into whichever top corner suits the ` +
    `composition. Text creeping into it makes the logo and the headline collide.\n` +
...
function chooseSceneMode(template) {
  // 2026-08-06:知识帖不掷骰子。掷骰子正是教育帖看起来像产品帖的根因 ——
  // 75% 的概率抽到 interior,模板写的 explainer 背景整个不用,出的是卧室实景。
  if (template && template.forceSceneMode) return template.forceSceneMode;
 ...
 */
/**
 * 重出时改写图上文字 —— 只改她要求改的那部分,其余逐字保留。
 * 她没提文字 → 三行原样返回;她说"标题换成 X" → 只有标题变。
 * 任何失败都返回 null,调用方沿用上一版(绝不退回"重新提炼一套")。
 */
async function reviseImageText(current, feedback, apiKey) {
  ...
  try {
    const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://fanz-marketing-bot.railway.app',
        'X-Title': 'Fanz Marketing Bot - Image Text Revision',
      },
      body: JSON.stringify({
        model: process.env.DESIGN_AGENT_MODEL || process.env.MODEL || 'gpt-4o',
        messages: [{
          role: 'system',
          content:
            'You are editing the short text printed on a marketing image. You are given the text ' +
            'that is on it now, and what the reviewer asked to change about the image.\n' +
            'DEFAULT TO CHANGING NOTHING. Reproduce each line EXACTLY as given unless the ' +
            'reviewer explicitly asked for different words. Asking for smaller text, a different ' +
            'colour, a different room or a different position is NOT a request to reword anything.\n' +
            'Output exactly these three lines and nothing else:\n' +
            'HEADLINE: ...\nSUBHEADING: ...\nCTA: ...\n' +
            'Leave a line empty only if it is empty now or the reviewer asked to remove it.',
        }, {
          role: 'user',
          content: `Current text on the image:\nHEADLINE: ${current.title || ''}\n`
            + `SUBHEADING: ${current.selling_point || ''}\nCTA: ${current.cta || ''}\n\n`
    ...
    if (!title) throw new Error('no HEADLINE in revision');
    ...
    console.error('[design-agent] 图上文字改写失败:', err.message);
    return null;
  } finally {
    clearTimeout(timer);
  }
...
async function deriveImageText(row, opts = {}) {
  ...
  // ── 重出时:图上文字默认**沿用上一版** ──
  //
  // 2026-08-06 实证抓到:她写 "…Keep the same fan and the same wording",
  // 字号/光线/构图都照办了,**文字却全换了**
  // (Lazy afternoon sanctuary → Lazy afternoon breeze)。
  // 因为提炼层每次重出都从正文重新压一遍短句,跟她的意见毫无关系。
  //
  // 这件事比看起来严重:她想要的往往只是"字小一点",代价却是丢掉一句她已经
  // 认可的标题 —— 于是"微调一下"变成"再赌一次"。
  // 默认沿用、只有她要求改文字时才改;这一层本来就要调一次 LLM,不额外花钱。
  const cur = opts.currentTexts;
  if (opts.imageFeedback && cur && cur.title) {
    const revised = await reviseImageText(cur, opts.imageFeedback, apiKey);
    if (revised) return revised;
    console.error('[design-agent] 图上文字改写失败,原样沿用上一版(总好过随机换一套)');
    return cur;
  }
...
async function buildReferenceImagePrompt(row, template, productName, brandVoice, productMeta, brandColors, opts = {}) {
  ...
  // ── 审核者对上一张的意见 ──
  //
  // 2026-08-06:这是这条路上第一次真的用上她的话。此前 [scene] 只被旧的合成链路
  // 读取,而现在除节庆外**全部**走这里 —— 她说"字太大",系统随机重出一张,
  // 界面还显示"已重新生成"。她会以为 AI 听不懂人话,而不是这个功能坏了。
  //
  // 放在最后:整张提示词前面都是"这类帖子一般怎么画",而这一段是"这一张
  // 具体哪里不对"。她看过成品、指名道姓提了要求,这比任何通用规则都优先。
  const feedbackBlock = opts.imageFeedback
    ? `\n\nREVIEWER FEEDBACK ON THE PREVIOUS VERSION OF THIS IMAGE — this overrides anything above ` +
      `that conflicts with it. A person looked at the last render and asked for this:\n` +
      `"${String(opts.imageFeedback).slice(0, 600)}"\n` +
      `Apply it literally. If she says the text is too large, set it visibly smaller than a normal ` +
      `layout would; if she names a colour, use that colour; if she says something sits too high or ` +
      `too low, move it. Keep everything she did NOT mention as close to the previous version as ` +
      `you can — she is asking for a fix, not a different image.`
    : '';

  // ── 知识帖:信息图当主体，面板内容取清单真值 ──
  // 取不到面板就**退回普通版式**，绝不出一张有框没内容的假信息图。
  if (template && template.tag === 'knowledge_explainer') {
    const { explainerPanel, explainerLayoutBlock } = require('./design-templates');
    const panel = explainerPanel(productMeta);
    if (panel) {
      const prompt =
        `Create a flat editorial infographic for a Malaysian ceiling fan brand, using the ceiling ` +
        `fan in the attached product image as a small supporting reference — keep its exact shape, ` +
        `colour and details 100% accurate wherever it appears.` +
        `${productSpecBlock(productMeta)}\n\n` +
        `Canvas & Background: ${direction}` +
        `${explainerLayoutBlock(panel, texts.title, brandBlue)}` +
        `${logoBlock(template.logoPosition)}${formatBlock()}${feedbackBlock}`;
      return {
        prompt, source, texts, sceneMode,
        logoSeries: pickLogoSeries(sceneMode, template),
        explainerPanel: panel,
      };
    }
    console.error('[design-agent] 知识版式拿不到面板真值，退回普通版式(不出假信息图)');
  }
  ...
  const prompt =
    ...
    `${typographyBlock(texts, brandBlue)}${logoBlock(template.logoPosition)}${formatBlock()}${feedbackBlock}`;
  return { prompt, source, texts, sceneMode, logoSeries: pickLogoSeries(sceneMode, template) };
}

module.exports = { buildReferenceImagePrompt, deriveCreativeDirection, deriveImageText, pickLogoSeries, TEXT_ZONE, TEXT_ZONE_TOP, FESTIVAL_TEXT_ZONE };
