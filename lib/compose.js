...
async function buildLogoLayer(W, H, opts) {
  ...
  // 先裁掉素材自带的透明留白,再按"有墨宽度"缩放。
  //
  // 2026-07-30 实测 bug:logo 素材是 2000x2000,但真正有墨的只有 997px 宽 ——
  // 一半是透明留白。原来直接 resize 到 W*ratio,那个 ratio 量的是**外框**,
  // 看得见的标记只有一半大(设 15% 实际只有 7.5%)。三轮"logo 太小/太大"的
  // 讨论其实都是在讨论这个算错的数字。裁掉留白后 ratio 才是它字面的意思。
  const logoW = Math.round(W * (opts.logoWidthRatio || brandKit.LOGO.widthRatio));
  let inkBytes = logoBytes;
  try {
    inkBytes = await sharp(logoBytes).trim({ threshold: 8 }).png().toBuffer();
  } catch (_) { /* 整图透明等边界情况:退回原图,不让 logo 层炸掉 */ }
  const logoBuffer = await sharp(inkBytes).resize(logoW, null, { fit: 'inside' }).png().toBuffer();
  ...
  // 极淡底衬(scrim)—— 和上面那个 withBackdrop 方板是两回事,别混用。
  //
  // 2026-08-03 Edwin:浅色/中间调背景上蓝版对比不够时,**不许退白版**(白版在
  // 浅底上是发虚),而是把背景稍微提亮一点点。要求是"很淡,不能变成方框",
  // 所以这里用**径向渐变椭圆**、边缘完全透明,视觉上是一层雾,不是一个色块。
  else if (opts.logoScrim) {
    // 2026-08-03 第二轮:第一版(0.50/0.34,半径 0.95×)Edwin 一眼看出「一圈浅色
    // 光晕、有圆形轮廓,像 logo 自带发光,显廉价」。
    // 光晕之所以看得见,不是因为它亮,是因为它**边缘收得太快** —— 半径小、
    // 中心浓,人眼就能描出那个圆。所以这一版反着调:峰值不透明度砍到不足四分之一,
    // 半径放大一倍多,让它摊平成一层察觉不到的雾。
    // Edwin 定的取舍:「宁可对比度只有 2.8:1 看起来干净,也不要 3.3:1 但能看出光圈」。
    const cx = left + lm.width / 2;
    const cy = top + lm.height / 2;
    const rx = Math.round(lm.width * 2.0);
    const ry = Math.round(lm.height * 2.0);
    const scrim = Buffer.from(
      ...
      `</radialGradient></defs>` +
      `<ellipse cx="${Math.round(cx)}" cy="${Math.round(cy)}" rx="${rx}" ry="${ry}" fill="url(#s)"/>` +
      `</svg>`
    );
...
async function composeFinal(opts) {
  ...
  const warnings = [];
  const productExpected = Boolean(opts.productSource || opts.productPath);
  const product = productExpected
    ? await prepareProductLayer(opts.productSource || opts.productPath, boxW, boxH)
    : null;
  if (product) {
    layers.push({
  ...
  } else if (productExpected) {
    // 2026-07-30 实测事故:这里原来只 console.warn 然后继续，结果整月批量里
    // 两张 educational 成品"没有风扇"却照样标记成功 —— 系统安静地把产品省掉了。
    // 素材库改成"分辨率优先"后是白底 JPG，没有真 alpha，正好踩中这条。
    // 现在:本该有产品却贴不上 = 硬失败。图少了产品就是废图，宁可失败让重试/
    // 通知机制介入，也不要交一张废图出去。
    throw new Error(
      `product layer could not be composited (${productName}): no real transparency in the ` +
      `product image. A composite-mode post without its product is unusable — fix the asset ` +
      `(transparent PNG) or route this template to ai_reference mode.`
    );
  }

  // Logo 层 — 品牌要素，贴不上要留痕，不能安静地出一张没 logo 的图
  try {
    const logoLayers = await buildLogoLayer(W, H, opts);
    if (logoLayers.length === 0) warnings.push('logo layer empty — no logo asset resolved');
    layers.push(...logoLayers);
  } catch (logoErr) {
    warnings.push(`logo layer failed: ${logoErr.message}`);
  ...
  // warnings 往上抛给 pipeline 落进 compose_spec + 审核卡，别让降级只活在日志里
  return { outPath: opts.outPath, width: W, height: H, warnings };
}
...
module.exports = {
  ...
  prepareProductLayer,
  loadBackground,
  loadAsset,        // pipeline 的 full_ai 分支要拿背景 buffer 去量 logo 落点
  applyLogoOverlay,
  applyBadgeOverlay,
  canRenderText,
};
