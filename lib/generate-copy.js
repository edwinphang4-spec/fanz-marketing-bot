 ...
 * @param {boolean} [opts.persist=true] 把定下来的角度/品牌事实/型号写回 row（干测时关掉）
...
async function generateCopy(opts) {
  const {
    ...
    callLLM, assignProduct = true, persist = true,
  } = opts;
  ...
  // 驳回重写时,把**她正在看的那一版**一起交给模型 —— 否则"重写"是从零再来一遍,
  // 她没意见的部分每次都会变,连点三次驳回会拿到三篇不相干的稿子(2026-08-06 查出)。
  const previousVersion = reviewNotes
    ? (opts.previousVersion || (row && (row.fb_content || row.ig_content)) || null)
    : null;

  const prompt = buildCopywritingPrompt(
    topic, pillar, reviewNotes, brandVoice, productCtx, angleCtx, postDate, previousVersion
  );
  ...
  // ── ④' 「改短一点」必须真的变短 ──
  //
  // 2026-08-06 干测抓到:她写"太长了,砍掉一半",重写回来 247 字 → 302 字,**更长了**。
  // 提示词里已经有"按意见改"和"保留她没抱怨的部分",但整段提示词其余部分(品牌声音、
  // 开场钩子、1-3 段的格式要求)在往反方向拉,模型顺着写就长了。
  //
  // 长度是这些意见里**唯一能被代码量准**的一项,所以这一项不靠祈祷。
  // 这里用关键词识别"她是不是在要求变短"是可以的:认错了最坏的结果只是"少做一次检查",
  // 不像事实拦截那样会放过一条假声明 —— 两种场景的代价不对等。
  const wantsShorter = reviewNotes && /shorter|too long|cut|trim|shorten|太长|长了|砍掉|精简|短一点/i.test(reviewNotes);
  if (wantsShorter && previousVersion) {
    const before = String(previousVersion).length;
    const after = String(parsed.fb_content || '').length;
    if (after >= before * 0.9) {
      console.warn(`[generate-copy] 要求改短但没变短(${before}→${after} 字),重来一次`);
      const raw = await callLLM([
        { role: 'system', content: prompt },
        { role: 'user', content: `${baseUser}\n\nYour rewrite came back ${after} characters long — the previous version was ${before}. She asked for it SHORTER. Cut it down: remove whole sentences, not adjectives. The Facebook version must be well under ${Math.round(before * 0.7)} characters. Keep the product and the point; drop everything else.` },
      ]);
      const shorter = parseCopywritingResponse(raw);
      const claims = shorter && checkFabricatedClaims(`${shorter.fb_content}\n${shorter.ig_content}`, assignedMeta);
      const ok = shorter && claims.ok && validateCopywritingResult(shorter).valid;
      if (ok && shorter.fb_content.length < after) parsed = shorter;
      else console.error('[generate-copy] 第二次仍未变短,交回较短的那一版由人判断');
    }
  }

  // ── ⑤ 把这一篇「定下来的东西」写回行上 ──
  //
  // 2026-08-05 又栽在同一个坑的另一半:上一轮收口只做了**上游**（所有生成路径
  // 都调这一个函数），没做**下游**（所有消费方都读同一个源）。结果是——
  // 角度在这里算出来了(knowledge)、文案也照着写了，但只有月度那条路把它写进
  // compose_spec；单篇路没写。出图提炼图上文字时 pipeline 从 compose_spec 读不到
  // 角度，回落到 spec(规格)角度，教育帖图上于是印了 "56-inch Diameter"
  // 而不是那条知识。文案对、图错，看起来像两个 bug，其实是同一个。
  //
  // 判断标准从此是两句，不是一句:
  //   上游:所有调用方都进同一个入口了吗?
  //   下游:所有消费方都读同一个源了吗?
  // 决定在这里做，就在这里落库 —— 调用方不需要记得，也就不可能忘。
  if (persist && row && row.id) {
    try {
      const patch = {};
      if (angleCtx) {
        patch.compose_spec = { ...spec, angle: angleCtx.angle, brand_fact: angleCtx.brandFact || null };
      }
      // 我们替这篇挑了型号 → 落库，出图时才不会另挑一台造成图文不符
      if (productName && !(row && row.source_product_image)) patch.source_product_image = productName;
      if (Object.keys(patch).length) {
        await require('./supabase').updateContentCalendar(row.id, patch);
        // 同一次请求里后续还会读 row，就地更新，免得读到旧值
        if (patch.compose_spec) row.compose_spec = patch.compose_spec;
        if (patch.source_product_image) row.source_product_image = patch.source_product_image;
      }
    } catch (err) {
      // 写不进不阻断出稿，但必须喊出来:下游会退回自己猜角度
      console.error('[generate-copy] 角度/型号写回失败，出图会回落到自行推断:', err.message);
    }
  }
