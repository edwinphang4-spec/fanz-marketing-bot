 ...
 */
function checkMonthlyRepetition(rows = []) {
  const alerts = [];
  const detail = {};
  ...
  for (const field of ['title', 'selling_point', 'cta']) {
    ...
    for (const o of over) {
      alerts.push(`图上${label}「${o.sample}」在整月出现 ${o.count} 次(上限 ${MAX_SAME_IMAGE_TEXT})`);
    }
    // 多样性本身也值得报:不重复率过低
    if (values.length >= 5 && t.size <= Math.ceil(values.length / 3)) {
      alerts.push(`图上${label}只有 ${t.size} 种写法(共 ${values.length} 篇)——多样性过低`);
    }
  }
  ...
  for (const o of dupOpen) {
    alerts.push(`开头第一句「${o.sample.slice(0, 60)}」重复 ${o.count} 篇`);
  }
