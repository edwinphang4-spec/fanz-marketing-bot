  ...
  try {
    ...
    try {
      if (template.mode === 'ai_reference') {
        ...
        const { prompt, texts: designTexts } = await buildReferenceImagePrompt(
          row, template, product.name, kit.brand_voice, product.meta
        );
        ...
        fs.writeFileSync(outPath, ref.buffer);
      } else {
