    ...
    try {
      if (template.mode === 'ai_reference') {
        ...
        const { prompt, texts: designTexts, sceneMode, logoSeries } = await buildReferenceImagePrompt(
          row, template, product.name, kit.brand_voice, product.meta, kit.colors, usedTexts
        );
        ...
        if (isFullAi) {
          try {
            ...
            const placement = await qa.pickLogoPlacement(bgBuf, {
              logoWidthRatio: template.logoWidthRatio || qa.LOGO_INK_RATIO_TARGET,
              preferredPosition: template.logoPosition,
              // 节庆图的排版文字是模型画的（full_ai），标题一律在中部 —— 同样把
              // 顶部留给 logo，避免 09-16 那种"标题压住 logo"。
              textZone: require('./design-agent').TEXT_ZONE,
            });
