...
async function runImageryPipeline(rowId, opts = {}) {
  try {
    ...
    // 上一版用的 logo 版本 —— 用来判断这次重出有没有换版(要告诉她为什么)
    const prevLogoSeries = spec.logo_series || null;
    ...
    try {
      if (template.mode === 'ai_reference') {
        ...
        // 这篇讲了哪个知识点 —— 下个月规划时据此避开(2026-08-07 跨月记忆)
        if (explainerPanel && explainerPanel.teachingKey) spec.teaching_key = explainerPanel.teachingKey;
        spec.design_prompt = prompt;
        spec.scene_mode = sceneMode;

        // ── 只组装提示词、不出图(部署探针用)──
        // 提示词到这里已经完全成型(含审核意见),但真正花钱的那一步还没发生。
        // 落库后原地返回,行退回 image_ready、marker 由调用方清掉。
        // 有了这个,验"线上有没有接上反馈"不用再烧一张图。
        if (opts.promptOnly) {
          spec.image_texts = designTexts;
          if (opts.imageFeedback) spec.prompt_only_probe = true;
          await saveSpec(rowId, spec);
          await updateImageRow(rowId, { image_status: 'generated' }, 'generating').catch(() => {});
          await supabase.updateContentCalendar(rowId, { status: 'image_ready' }).catch(() => {});
          return { success: true, promptOnly: true, prompt };
        }
        ...
        // logo 版本跟着画面明暗走。她改了场景之后 logo 会从蓝版变白版 ——
        // 这是可读性在起作用(深底上蓝 logo 看不见),但她没预期到,看起来像出 bug。
        // 2026-08-06 Edwin:可读性优先于一致性,但要**顺口说明一句**,
        // 让她知道这是系统在照顾她。
        if (prevLogoSeries && prevLogoSeries !== logoSeries && opts.imageFeedback) {
          const toWhite = logoSeries === 'lockup_white';
          warnings.push(toWhite
            ? '画面变暗了,logo 自动换成白版保证清晰'
            : '画面变亮了,logo 自动换回品牌蓝版');
        }
