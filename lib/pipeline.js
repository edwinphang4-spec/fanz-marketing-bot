  ...
  try {
    ...
    try {
      if (template.mode === 'ai_reference') {
        ...
        const { prompt, texts: designTexts, sceneMode, logoSeries } = await buildReferenceImagePrompt(
        ...
        const ref = await generateReferenceImage({
          prompt,
        ...
        // 贴真实 logo。withBackdrop:false —— 不要底板方框(Edwin 明确否掉),
        // 靠 prompt 里"该角落留空"保证可读性。贴图失败不阻断,退回无 logo 成品。
        //
        // 蓝版/白版不再按场景模式猜,而是量 logo 落点的真实像素亮度——室内也
        // 可能是傍晚暗房,猜错就又变成"logo 压在暗墙上看不清"。
        let finalBuffer = ref.buffer;
        const logoRatio = template.logoWidthRatio || 0.15;
        try {
          const { applyLogoOverlay, pickLogoVariantByBrightness } = require('./compose');
          const { getLogoAssetBySeries } = require('./brand');
          const variant = await pickLogoVariantByBrightness(ref.buffer, {
            logoPosition: template.logoPosition,
            logoWidthRatio: logoRatio,
          });
          const wantSeries = variant === 'blue' ? 'lockup_blue' : 'lockup_white';
          if (wantSeries !== logoSeries) {
            const la = await getLogoAssetBySeries(wantSeries);
            if (la && la.public_url) {
              console.log(`[pipeline] logo variant corrected by measured brightness: ${logoSeries} -> ${wantSeries}`);
        ...
        if (!reuseBackground) {
          const bg = await generateBackground(row, opts.topicOverride || null, kit.background_style, template);
          if (!bg.success) {
            await releaseClaim(rowId);
