...
async function resolveAssetByModel(said) {
  ...
  //    2026-08-08:守卫从 >=3 放宽到 >=2 —— "FS" 正好 2 个字符,原来会被挡掉,
  //    于是"推 FS 的"得到"库里没有 FS 这台",而 FS 是真实系列(FS423L/FS48L/…)。
  //    收窄面很小:池里只有 FS 一个 2 字符系列名,其余都 4 个字符起。
  //    匹配到多台时调用方会问她是哪一台,不会替她挑。
  const exactModel = hits.length > 0;
  if (!hits.length && want.length >= 2) {
