...
async function pickProductsForPlan(rows) {
  ...
  for (const row of rows) {
    ...
    let cands = pool;
    if (hints.series) cands = narrow(cands, (a) => a.series === hints.series);
    ...
    if (hints.spaces.length) {
      cands = narrow(cands, (a) => {
        const sp = a.metadata.spaces || [];
        return hints.spaces.some((s) => sp.includes(s));
      });
    }
    ...
    const brandNew = usable.filter(
      (a) => !usedSeriesSize.has(`${a.series}|${a.metadata.size_inches}`)
    );
    const finalSet = brandNew.length ? brandNew : usable;

    // 同分里按 row 稳定取模（可复现），没有 id 时用 topic
    const pick = finalSet[hashOf(row.id || row.topic || '') % finalSet.length];
    usedNames.add(pick.name);
    usedSeriesSize.add(`${pick.series}|${pick.metadata.size_inches}`);
...
module.exports = { pickProductsForPlan, readHints, copywritingProductContext };
