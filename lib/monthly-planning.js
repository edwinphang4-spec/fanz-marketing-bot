...
const RATIO_BASELINE = Object.values(REQUIRED_RATIOS).reduce((a, b) => a + b, 0); // = 12

/**
 * 按批量缩放 pillar 配比(2026-08-08)。
 *
 * 用最大余数法而不是各自四舍五入 —— 后者的和不等于 n(3 篇会算出 4 篇),
 * 而"你要 3 篇我给你 4 篇"是她一眼就会发现的错。
 * 硬性验收:n=12 必须原样还原成 4/3/2/2/1。
 */
function pillarPlan(n) {
  const count = Math.max(1, Math.round(Number(n) || 0));
  const entries = Object.entries(REQUIRED_RATIOS).map(([k, base]) => {
    const exact = base * count / RATIO_BASELINE;
    return { k, base, floor: Math.floor(exact), frac: exact - Math.floor(exact) };
  });
  let left = count - entries.reduce((a, e) => a + e.floor, 0);
  // 余数按小数部分从大到小分;完全打平时按原始配比大的优先(product 先拿到)
  const order = [...entries].sort((a, b) => b.frac - a.frac || b.base - a.base);
  for (const e of order) { if (left <= 0) break; e.floor++; left--; }
  return Object.fromEntries(entries.map((e) => [e.k, e.floor]));
}
 ...
 */
/** 'September 2026' → 整月的 request(旧入口靠它转成新模型) */
function monthRequest(targetMonthStr) {
  const [monthName, yearStr] = String(targetMonthStr).split(' ');
  const mi = MONTHS.indexOf(monthName);
  const y = parseInt(yearStr, 10);
  const days = new Date(y, mi + 1, 0).getDate();
  const pad = (n) => String(n).padStart(2, '0');
  return {
    count: RATIO_BASELINE,
    from: `${y}-${pad(mi + 1)}-01`,
    to: `${y}-${pad(mi + 1)}-${pad(days)}`,
    label: targetMonthStr,
    monthLabel: targetMonthStr,
  };
}

/**
 * 提示词构建器 —— 2026-08-08 起收 request 对象而不是月份字符串。
 *
 * "整月"变成它的一个特例(count=12、from/to=当月首尾)。**内部只有这一条路** ——
 * 保留两套实现的话,"整月"和"N 篇"会慢慢漂开,和之前"新逻辑只接了月度批量"
 * 是同一个病。
 *
 * @param {object|string} input request 对象;传字符串则按旧的"整月"用法处理
 */
async function buildContentPlanPrompt(input) {
  const req = typeof input === 'string' ? monthRequest(input) : input;
  const targetMonthStr = req.monthLabel || `${req.from} ~ ${req.to}`;
  const regularCount = Math.max(1, Math.round(req.count || 12));
  const rangeLabel = req.label && req.label !== '指定日期'
    ? `${req.label} (${req.from} to ${req.to})`
    : `${req.from} to ${req.to}`;
  // 节庆帖只在批量够大时才加 —— 3 篇里塞一篇节庆,她要的 3 篇就只剩 2 篇正文。
  const allowFestival = regularCount >= 8;
  const focusBits = [];
  if (req.products && req.products.length) {
    focusBits.push(`The user specifically asked for these models: ${req.products.join(', ')}. Feature them.`);
  }
  if (req.pillars && req.pillars.length) {
    focusBits.push(`The user leans towards these post types: ${req.pillars.join(', ')}.`);
  }
  if (req.theme) focusBits.push(`Theme the user asked for: "${req.theme}".`);
  const focusContext = focusBits.length ? `\n\nWHAT THE USER ASKED FOR:\n- ${focusBits.join('\n- ')}` : '';
...
Your job: Generate a content calendar for ${rangeLabel}. Create exactly ${regularCount} regular post${regularCount === 1 ? '' : 's'} that hit the pillar counts below${allowFestival ? ', plus 0-2 extra festival posts' : ''}.

EVERY suggested_date MUST fall between ${req.from} and ${req.to} inclusive. Dates outside that window are rejected.

CURRENT DATE (Malaysia): ${currentDate}
PLANNING WINDOW: ${req.from} to ${req.to}${festivalContext}${coveredContext}${focusContext}
...
${Object.entries(pillarPlan(regularCount)).filter(([, v]) => v > 0).map(([k, v]) => `- ${k}: ${v} post${v === 1 ? '' : 's'}`).join('\n')}
${allowFestival ? `- festival: 0-2 extra posts (do NOT count toward the ${regularCount} regular posts)\n` : ''}
Total regular posts: ${regularCount}.${allowFestival ? ' Festival posts are additional.' : ' Do NOT add festival posts — this batch is too small for them.'}

RHYTHM (spread the posts evenly across the window):
- Spread them across the whole window; do not cluster them in the first few days
...
/** 旧入口:内部转成 request 走同一条路(不是第二套实现) */
async function buildMonthlySystemPrompt(targetMonthStr) {
  return buildContentPlanPrompt(monthRequest(targetMonthStr));
}
