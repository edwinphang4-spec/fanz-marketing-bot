 ...
 * @param {string|Date} [postDate] - **这篇帖子的 suggested_date**，不是今天。
 *   2026-08-01 干测实测事故：不传的话整月文案都以为"现在是生成的那一天"，
 *   一篇排在 9/28 的帖子写出 "Malaysia's National Day is near"（那时 8/31 和 9/16
 *   都已经过去了）。老板娘一眼就会打回，而且这是**结构性**的：下个月排期必然重犯。
 *   即兴单篇生成不传是对的（那时"今天"就是发布日）。
 */
function buildCopywritingPrompt(topic, pillar, reviewNotes, brandVoice, productContext, angleContext, postDate, previousVersion) {
  const planning = require('./planning');
  const when = planning.toPostDate(postDate);
  const currentDate = `${MONTHS[when.getMonth()]} ${when.getDate()}, ${when.getFullYear()}`;
  const activeSeasons = planning.seasonalContextFor(when);
  ...
  // ── 知识帖:文案必须和配图那张对比表说同一件事 ──
  //
  // 2026-08-06 实测事故:图上标题印 "More Blades Myth"(说这是个误解),
  // 正下方的对比表印 "more blades move more air"(说这是真的)。一张图自相矛盾,
  // 而且**看起来很专业** —— 这类错最危险,老板娘不会怀疑它。
  //
  // 根因:配图的面板是从清单真值算出来的,文案却在自由发挥,两边没有共同的结论。
  // 面板是真值,所以以面板为准:把结论先给文案,并明确不许唱反调。
  // "反转钩子"只有在数据真的反转时才成立 —— 我们四个尺寸段全是同方向,没有反转可讲。
  let teachingBlock = '';
  if (angleContext && angleContext.angle === 'knowledge' && a) {
    try {
      const panel = require('./design-templates').explainerPanel(a);
      if (panel) {
        teachingBlock =
          `\n\nTHIS POST'S TEACHING POINT — the image for this post carries a comparison panel built ` +
          `from Fanz's own published figures, and it will say:\n` +
          `  "${panel.caption}" — ${panel.left.join(' / ')}  vs  ${panel.right.join(' / ')}\n` +
          `Your copy must teach THAT SAME conclusion. Do not argue the opposite, do not open with ` +
          `"it's a myth that…" or "not always" — the figures are real and they point one way. ` +
          `A headline that contradicts the panel underneath it makes the post look expert and be wrong, ` +
          `which is worse than being plainly wrong. You may name both models and both figures.`;
      }
    } catch (_) { /* 算不出面板就不加这一段,文案照常写 */ }
  }
...
${rangeBlock}${assignedBlock}${seriesFacts}${sizeGuide}${teachingBlock}
${factBoundary}

WHEN THIS POST GOES OUT: ${currentDate} (Malaysia)
Write as if it is that date — not today. Anything you call "now", "this week", "coming up"
or "just around the corner" is measured from the publish date above.

SEASONAL / FESTIVAL CONTEXT FOR THAT DATE:
${activeSeasons}
...
Brand slogan: #TheAirMover
  (2026-08-05:#TheAirExpert 已停用。Fanz 的 IG 签名、FB 页名、以及我们自己
   素材库里所有 logo 上写的都是 "THE AIR MOVER" —— 三处一致。
   #TheAirExpert 查不到任何出处，很可能一直是错的。等 Fanz 确认两个是否都在用，
   说在用再加回来。用错品牌口号发出去是很尴尬的事，先按证据强的那个走。)
...
educational — Practical how-to. Help first, sell second. Soft CTA.
  STRUCTURE (2026-08-06, from a real post that failed review): the teaching must be the BULK of
  the post, not its opening line. Roughly: give the useful thing across the first two thirds —
  the rule, the comparison, what actually changes and why it matters to the reader — and let the
  product appear near the end in ONE sentence, as an example of the point you just made.
  The post that failed did the opposite: one line of knowledge, then a whole paragraph selling
  the model. A reader who already owns a fan should still finish it having learned something.
  Never open with the model name.
...
- Did you stay inside this post's CONTENT ANGLE, and use at most the ONE brand fact it was
  ...
  if (reviewNotes && typeof reviewNotes === 'string' && reviewNotes.trim()) {
    // 2026-08-06:这一段原本只给意见,**不给上一版正文** —— 于是"重写"实际上是
    // 拿着一句"太长了"从零再写一遍,而不是改这一版。她想保留的部分每次都会变,
    // 她连点三次驳回,拿到的是三篇不相干的稿子。
    const { formatForPrompt } = require('./review-notes');
    const notesBlock = formatForPrompt(reviewNotes) || reviewNotes.trim();
    const prev = (previousVersion && String(previousVersion).trim()) || '';
    prompt += `\n\nREVISION — this post already exists and the reviewer wants it changed.\n${notesBlock}`;
    if (prev) {
      prompt += `\n\nTHE VERSION SHE IS LOOKING AT:\n"""\n${prev.slice(0, 1500)}\n"""\n` +
        `EDIT THAT VERSION. Keep everything she did not complain about — the angle, the ` +
        `structure, the product, the phrasing that works — and change only what the notes ask for. ` +
        `Returning a completely different post is a failure even if the new post is good: she ` +
        `approved of everything except the point she raised, and rewriting from scratch throws ` +
        `that away and makes her re-read the whole thing.\n` +
        `If a note asks for it shorter, the result must be VISIBLY shorter than the version above — ` +
        `delete whole sentences rather than trimming adjectives. Everything else in this brief ` +
        `(voice, opening hook, paragraph count) yields to her notes: she has read this post and ` +
        `told you what is wrong with it, and that beats a general style rule.`;
    } else {
