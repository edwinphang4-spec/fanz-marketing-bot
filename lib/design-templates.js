...
function explainerPanel(productMeta) {
  ...
  if (pair) {
    return {
      ...
      // 讲过什么要留痕,否则下个月还会再讲一遍(2026-08-07 跨月记忆)。
      // 按尺寸区分:56 吋讲过叶数对比,不代表 66 吋那篇也不能讲。
      teachingKey: `blade_count_airflow_${pair.size}in`,
  ...
  if (sibling && pc.canCite(model, 'led')) {
    ...
    return {
      kind: 'light_choice',
      teachingKey: `light_vs_no_light_${spec.size_inch}in`,
 ...
 */
function buildFullAiPrompt(row, template, reviewerFeedback) {
  const headline = (row && row.topic) || 'Festive Greetings';
  const angle = (row && row.post_angle) ? `\nContext for the design mood: ${row.post_angle}` : '';
  // 审核者对上一版的意见 —— 排在最后,压过上面所有通用规则(2026-08-06)
  const fb = reviewerFeedback
    ? `\n- REVIEWER FEEDBACK on the previous version, apply it literally and keep everything else ` +
      `as close to that version as you can: "${String(reviewerFeedback).slice(0, 400)}"`
    : '';
  return (
