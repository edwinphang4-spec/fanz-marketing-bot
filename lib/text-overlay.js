...
/**
 * Calculate how many CJK characters fit per line given a pixel width.
 * Assumes CJK chars are roughly fontSize px wide, latin chars ~0.6 * fontSize.
 */
function charsPerLine(maxWidthPx, fontSize) {
  // Conservative estimate: treat all as CJK-width (1em each)
  return Math.max(1, Math.floor(maxWidthPx / fontSize));
}
...
function buildTextSvg(lines, preset, imageWidth, imageHeight) {
  ...
  if (preset.backgroundOpacity && preset.backgroundOpacity > 0) {
    ...
    const rectY = Math.round(posY - rectPadding);
    const rectH = Math.round(textBlockH + rectPadding * 2);
...
async function applyTextOverlays(imagePath, texts, outputPath, options = {}) {
  ...
  for (const [presetName, text] of Object.entries(texts)) {
    ...
    const cpl = charsPerLine(availableWidth, preset.fontSize);
