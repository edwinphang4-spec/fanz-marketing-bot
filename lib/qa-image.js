...
async function pickLogoPlacement(imageBuffer, opts = {}) {
  ...
  for (const pos of candidates) {
    try {
      ...
      const overlapsText = (() => {
        if (!opts.textZone) return false;
        const zTop = (opts.textZone.top || 0) * meta.height;
        const gap = Math.round(meta.height * LOGO_TEXT_MIN_GAP);
        return (box.top + box.height + gap) > zTop;
      })();
