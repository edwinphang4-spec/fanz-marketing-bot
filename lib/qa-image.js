...
const BLUE_CONTRAST_SCRIM = 2.0;  // 蓝版低于此值,淡底衬也救不回来 → 才考虑挪位
...
async function pickLogoPlacement(imageBuffer, opts = {}) {
  ...
  // ③ 默认位差得离谱(连淡底衬都救不回来)→ 优先挪到蓝版真正够用的角落
  const blueUsable = scored
    .filter((s) => s.cBlue >= GRAPHIC_CONTRAST_MIN && s.busyness <= BUSY_OK)
    .sort((a, b) => b.cBlue - a.cBlue);
  if ((!def || def.cBlue < BLUE_CONTRAST_SCRIM) && blueUsable.length) {
    return out(blueUsable[0], 'blue', {
      moved: blueUsable[0].position !== preferred,
      reason: def
        ? `default ${preferred} unusable for blue (${def.cBlue}:1) — moved to a corner where blue works`
        : `default ${preferred} unmeasurable — moved to a corner where blue works`,
    });
  }

  // ④ 默认位蓝版偏弱但有救 → 留在默认位,加极淡底衬把背景提亮
  //    位置稳定比多挣那点对比度重要:logo 在帖子之间乱跳更伤品牌。
  if (def && def.cBlue >= BLUE_CONTRAST_SCRIM) {
    return out(def, 'blue', {
      scrim: true,
      reason: `blue ${def.cBlue}:1 on a light/mid background — kept in place with a soft scrim (never fall back to white here)`,
    });
  }

  // ⑤ 兜底:哪个角落蓝版最好就去哪,并加底衬
  const bestBlue = [...scored].sort((a, b) =>
    (a.busyness <= BUSY_OK ? 0 : 1) - (b.busyness <= BUSY_OK ? 0 : 1) || b.cBlue - a.cBlue)[0];
  return out(bestBlue, 'blue', {
    moved: bestBlue.position !== preferred,
    scrim: true,
    reason: `no corner clears blue ${GRAPHIC_CONTRAST_MIN}:1 — best available (${bestBlue.cBlue}:1) with a soft scrim`,
  });
}
