...
// logo 底部与文字区上沿之间至少要留的垂直空隙(占画布高)。
// 实测分界很清楚:Edwin 认可的三张留白 22–27%(09-01 22 / 09-03 27 / 09-29 25),
// 他判"挤成一团"的 09-30 只有 12%。取 18% 落在两者中间。
// 注意这是拿 4 张样本定的阈值 —— 不是普适常数,以后样本多了要回头校。
// 常态下并不会触发:提示词已规定文字排在 42% 以下,顶部角落天然留白 24%。
// 它的作用是**模型不听话时的安全网**。
const LOGO_TEXT_MIN_GAP = 0.18;
...
async function pickLogoPlacement(imageBuffer, opts = {}) {
  ...
  for (const pos of candidates) {
    try {
      ...
      // 这个角落会不会压到文字区。
      //
      // 2026-08-03 Edwin:09-30 挪到左上后紧接着就是标题,左上挤成一团、右上空着。
      // 我试过三种从成品图里**探测**文字位置的办法,全部失败:
      //   · OCR:09-30 一行都没读到(白字压暖色浅底,tesseract 认不出)
      //   · 大尺度灰度方差:09-30 左边 31.0 反而比右边 33.5 "干净"
      //   · 细尺度边缘能量:四张全报警,分不开窗帘边框和文字笔画
      // 结论:探测不可靠,别猜 —— 改成**由构造得知**。文字区是我们在提示词里
      // 规定给模型的(design-agent 的 TEXT_ZONE_TOP),所以这里直接拿来排除,
      // 不需要任何检测。
      // 注意判据是**垂直留白**,不是几何相交。
      // 09-30 实测:logo 框底在画布 18%、标题从 30% 起 —— 两者根本不相交,
      // 按"重叠"判会放行,但视觉上就是挤成一团。而 Edwin 认可的 09-03 是
      // logo 底 18% / 标题 45%,留白 27%。所以真正的分界是"隔多远",不是"碰没碰到"。
      // 2026-08-06:文字区从「上边界」改成「上下都有的带」。
      //
      // 旧写法只有 top,等于假设"文字永远在下半部" —— 对参考图那条路成立,
      // 对节庆图不成立(它的排版文字是居中的)。实测马来西亚日那张:四个角
      // **全部**被判成压文字,包括本来干净的下方两角。判定于是无路可走,
      // 退回默认位置,看起来像"判断对了",其实守卫根本没起作用。
      //
      // gap 也改成由文字区自己声明:0.18 是拿参考图 4 张样本定的,
      // 套到居中排版上会把整张画布吞掉。
      const overlapsText = (() => {
        const z = opts.textZone;
        if (!z) return false;
        const gap = Math.round(meta.height * (z.gap != null ? z.gap : LOGO_TEXT_MIN_GAP));
        const zTop = (z.top || 0) * meta.height - gap;
        const zBottom = (z.bottom != null ? z.bottom : 1) * meta.height + gap;
        // logo 框和「文字带 + 间距」在垂直方向相交才算压到
        return (box.top + box.height) > zTop && box.top < zBottom;
      })();
...
function decideLogoPlacement(scored, preferred = 'top_right') {
  ...
  // 压到文字区的角落一律不作为**挪位目标**。
  // Edwin 定的取舍:「宁可留在原位加淡底衬,也不要挪到文字上面」——
  // 所以只从挪位候选里剔除,默认位即使压文字也仍然可以留(它本来就在那儿)。
  const placeable = scored.filter((s) => !s.overlapsText);
  ...
  if (def && def.luminance <= DARK_BG_LUMINANCE) {
    // 暗背景挪位同样不许挪到文字上
    const darkPool = placeable.filter((s) => s.luminance <= DARK_BG_LUMINANCE);
    ...
    const best = [...(darkPool.length ? darkPool : (placeable.length ? placeable : scored))]
  ...
  // ② 默认位蓝版就够用 **且不压文字** → 最理想,原地不动、不加底衬
  //
  // 2026-08-06 补上 !overlapsText 这个条件。原来这一支只看对比度和杂乱度,
  // 默认位压不压文字压根不参与判断 —— 实测节庆图:默认右上角对比 7.95:1、
  // 干净度 0.8,直接命中这一支返回,而那个角落正压着双子塔插画。
  // 「默认位即使压文字也可以留」当初是为了避免瞎挪位,但那说的是**没有更好
  // 选择时**;有干净角落可去却不去,是错的。
  if (def && def.cBlue >= GRAPHIC_CONTRAST_MIN && def.busyness <= BUSY_OK && !def.overlapsText) {
    return out(def, 'blue', { reason: `default position kept (blue ${def.cBlue}:1)` });
  }

  // ②' 一个不压文字的角落都没有 → 这不是"照旧",是要说出来的状态。
  //     留在默认位,但加淡底衬保证读得出来,并把原因如实记下来。
  if (placeable.length === 0) {
    const fallback = def || [...scored].sort((a, b) => b.cBlue - a.cBlue)[0];
    return out(fallback, 'blue', {
      scrim: true,
      reason: `no corner free of text — kept ${fallback.position} with a scrim (blue ${fallback.cBlue}:1)`,
    });
  }
  ...
  const cleanCorners = placeable.filter((s) => s.busyness <= BUSY_OK);
  const bestElsewhere = [...(cleanCorners.length ? cleanCorners : placeable)]
  ...
  // 兜底也不许挪到文字上:候选仍然只从 placeable 里取,取不到就留在默认位。
  const bestBlue = bestElsewhere || def || placeable[0] || scored[0];
