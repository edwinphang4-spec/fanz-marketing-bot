 ...
 *
 * 判定标准两条,都是可算的:
 *   ① 对比度:品牌蓝或白至少一版够 WCAG 图形门槛 3:1
 *   ② 干净度:该区域边缘能量越低越好(压在家具/文字上就不干净)
 * 取"够对比 且 最干净"的那个;都不够对比就取对比最高的。
 *
 * @returns {Promise<{position:string, variant:'blue'|'white', contrast:number, busyness:number}>}
 */
async function pickLogoPlacement(imageBuffer, opts = {}) {
  ...
  for (const pos of candidates) {
    try {
      ...
      const cWhite = contrastRatio(WHITE_RGB, bg);
      const variant = cBlue >= GRAPHIC_CONTRAST_MIN ? 'blue' : (cBlue >= cWhite ? 'blue' : 'white');
      const contrast = variant === 'blue' ? cBlue : cWhite;

      // 干净度:灰度标准差 —— 纯净的墙面接近 0,压在家具/图案上会很高
      const grey = await regionStats(imageBuffer, box, true);
      const busyness = grey.channels[0].stdev;

      scored.push({ position: pos, variant, contrast: +contrast.toFixed(2), busyness: +busyness.toFixed(1), bgRgb: bg });
    } catch (_) { /* 该位置取不到就跳过 */ }
  }
  if (scored.length === 0) return { position: 'top_right', variant: 'blue', contrast: null, busyness: null };

  // 排序策略:**默认位置优先,只有默认真的不行才挪**。
  // 2026-08-01 实测教训:一开始按"杂乱度最低"排,结果两个角落 3.9 vs 4.9
  // 这种噪声级差距也会让 logo 换位置 —— logo 在帖子之间乱跳比固定位置更伤品牌。
  // Fanz 自己也是有默认位、构图需要时才挪。
  const BUSY_OK = 25;                    // 低于这个值都算"干净",不再细分
  const preferred = opts.preferredPosition || 'top_right';
  const usable = scored.filter((s) => s.contrast >= GRAPHIC_CONTRAST_MIN);
  const pool = usable.length ? usable : scored;

  const def = pool.find((s) => s.position === preferred);
  // 默认位既够对比又够干净 → 直接用,不折腾
  if (def && def.contrast >= GRAPHIC_CONTRAST_MIN && def.busyness <= BUSY_OK) {
    return { ...def, movedFromDefault: false, all: scored };
  }
  // 否则才挑替代位:先看干净度分档,同档内优先品牌蓝,再按对比度
  pool.sort((a, b) =>
    (a.busyness <= BUSY_OK ? 0 : 1) - (b.busyness <= BUSY_OK ? 0 : 1)
    || (b.variant === 'blue') - (a.variant === 'blue')
    || b.contrast - a.contrast
  );
  return {
    ...pool[0],
    movedFromDefault: pool[0].position !== preferred,
    reason: def
      ? `default ${preferred} rejected (contrast ${def.contrast}:1, busyness ${def.busyness})`
      : `default ${preferred} unmeasurable`,
    all: scored,
  };
}
