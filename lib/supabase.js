...
const VALID_PILLARS = ['product', 'case', 'promo', 'story', 'educational'];
...
async function listContentCalendar(filter = {}) {
  ...
  if (filter.plan_id) params.set('plan_id', `eq.${filter.plan_id}`);
  // 跨月记忆用:取"最近 N 天"。suggested_date 可能为空(单篇即兴生成没有排期),
  // 所以两个日期字段任一落在窗口内都算 —— 只看 suggested_date 会漏掉单篇。
  if (filter.since) {
    params.set('or', `(suggested_date.gte.${filter.since},created_at.gte.${filter.since})`);
  }
...
async function listContentCalendarByPlanId(planId) {
  if (!planId) throw new Error('listContentCalendarByPlanId: plan_id is required');
  return listContentCalendar({ plan_id: planId });
}
