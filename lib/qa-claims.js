...
function checkFabricatedClaims(text, productMeta = null) {
  ...
  const masked = maskBrandFacts(raw);
  ...
  const inches = [...raw.matchAll(/\b(\d{2})\s*(?:["”]|inch(?:es)?\b|-inch\b)/gi)].map((m) => Number(m[1]));
  ...
  const blades = [...raw.matchAll(/\b(\d)\s*[-\s]?blades?\b/gi)].map((m) => Number(m[1]));
