 ...
 * 不是"长得像风量的字符串"。
 */
function checkVerifiedSpecs(masked, productMeta, blocking, warnings, raw) {
  ...
  // 对比帖会**正当地**写出另一台扇的风量。
  //
  // 2026-08-06 实测:文案按要求写 "GAZE52N 7,539 CFM vs INNO525L 10,000 CFM",
  // 结果被拦成"风量对不上:文里写 10,000 但 GAZE52N 是 7539" —— 两次重写都被拦,
  // 整篇生成失败。这是我上一处改动带出来的:提示词让文案引用对比方的数字,
  // 拦截器却只认本篇这一台。
  //
  // 放行条件收紧到两条同时成立:
  //   ① 那台必须是清单里**和本篇同尺寸、真的构成对比配对**的型号(不是随便哪台);
  //   ② 那台的**型号名必须出现在文里** —— 光写个漂亮数字不算,读者得能自己核。
  const partnerCfm = new Map();  // cfm → 型号名
  if (found) {
    try {
      for (const c of found.pc.airflowComparisons()) {
        if (c.more.model === found.model) partnerCfm.set(Number(c.fewer.cfm), c.fewer.model);
        else if (c.fewer.model === found.model) partnerCfm.set(Number(c.more.cfm), c.more.model);
      }
    } catch (_) { /* 算不出配对 → 一台都不放行 */ }
  }
  const namesInText = String(raw).toUpperCase().replace(/[^A-Z0-9]/g, '');
  const isNamedPartner = (n) => {
    const model = partnerCfm.get(n);
    if (!model) return false;
    return namesInText.includes(String(model).toUpperCase().replace(/[^A-Z0-9]/g, ''));
  };

  let out = masked;
  out = scan(/\b(\d[\d,]*)\s*cfm\b/gi, 'cfm', '风量',
    (n, real) => n === Number(real) || isNamedPartner(n));
...
function checkFabricatedClaims(text, productMeta = null) {
  ...
  masked = checkVerifiedSpecs(masked, productMeta, blocking, warnings, raw);
  ...
  // 别台扇的叶数,只有在**那台被指名道姓写进文里**时才放行 —— 和风量用同一条规矩。
  //
  // 2026-08-06 第二次栽在这:上一版靠关键词判"这是不是对比句"
  // (than / vs / compared…),而文案写的是 "while the 5-blade INNO525L…",
  // 一个关键词都没命中,整篇被拦。关键词匹配本来就是猜;改成认型号名 ——
  // 文里点了 INNO525L,清单说它 5 叶,那"5 叶"就是可核的,与措辞无关。
  const namedBlades = (() => {
    const s = new Set();
    try {
      const pc = require('./product-catalog');
      const size = productMeta && Number(productMeta.size_inches);
      const flat = String(masked).toUpperCase().replace(/[^A-Z0-9]/g, '');
      if (size) for (const [m, spec] of Object.entries(pc.CATALOG)) {
        if (spec.size_inch !== size || !pc.canCite(m, 'blades')) continue;
        if (flat.includes(String(m).toUpperCase().replace(/[^A-Z0-9]/g, ''))) s.add(Number(spec.blades));
      }
  ...
  for (const hit of bladeHits) {
    ...
    if (namedBlades.has(n)) continue;   // 文里点名的同尺寸型号,清单说它就是这么多叶
