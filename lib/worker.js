...
async function processPlansInProduction() {
  ...
  for (const plan of plans) {
    ...
    let processed = 0;
    const copyResults = await runPool(copyQueue, IMAGE_CONCURRENCY, row => processRow(row, false));
    const retryResults = await runPool(retryQueue, IMAGE_CONCURRENCY, row => processRow(row, true));
