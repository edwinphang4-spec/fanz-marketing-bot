// ============================================
// worker.js — M-5/M-6/M-7 background worker
//
// Runs inside the bot process (single Railway instance) on a fixed tick.
// Consumes DB state written by the Dashboard, so Dashboard and bot stay
// decoupled (no cross-service HTTP):
//
//   1. content_plans.status='in_production'  (set by Dashboard "Start Image
//      Generation" button) -> batch-generate imagery for every copy_approved
//      row in the plan, sequentially, then notify the plan's chat_id.
//   2. content_calendar.status='image_retry' (set by Dashboard image review
//      reject / change-scene / change-product, or left behind by Telegram
//      flow) -> regenerate. Requested scene/product changes are carried in
//      review_notes markers: "[scene] <description>" or "[product-next]".
//   3. When every active post in an in_production plan reaches
//      approved/published -> auto-run M-6 schedulePlan and notify.
//   4. Daily MYT reminder tick (M-7) -> checkTodayPosts with real senders.
//
// Concurrency model: exactly one bot process runs in production, so an
// in-memory `processing` Set is the claim mechanism. index.js's inline
// Telegram regeneration shares this Set to avoid double generation.
// Rows left in image_status='generating' by a crashed process are reset
// to 'failed' on startup (crash recovery).
// ============================================

const supabase = require('./supabase');
const supabasePlans = require('./supabase-plans');
const { runImageryPipeline } = require('./pipeline');
const { schedulePlan, formatScheduleTable } = require('./monthly-scheduler');
const { checkTodayPosts, getMalaysiaDateStr } = require('../cron-publish-reminder');
const { listProductImages, writeSourceProductImage } = require('./select-product');
const { updateImageRow } = require('./image-state');
const { generatePostVideo } = require('./video-gen');

const TICK_MS = 45_000;
const FIRST_TICK_DELAY_MS = 5_000;
const REMINDER_HOUR_MYT = 9;
const MAX_FAILURES_PER_ROW = 3; // per process lifetime; bounds API spend on persistent failures
// GPT Image 2 calls are independent (~70-85s each at quality=medium); running
// them one by one made a 13-post batch take ~18 minutes. Modest parallelism
// keeps within OpenAI rate limits while cutting wall time to ~1/3.
const IMAGE_CONCURRENCY = Math.max(1, parseInt(process.env.IMAGE_CONCURRENCY || '3', 10) || 3);

// rowIds currently being generated (shared with index.js inline retry flow)
const processing = new Set();
// planIds we already sent a "batch started" notice for (per process)
const notifiedBatchStart = new Set();
// rowId -> consecutive failure count (per process)
const failCounts = new Map();
// rowIds we already sent a "failed 3 times" notice for
const notifiedFailure = new Set();

// ── 每周动态视频（方案 B）──
// rowId currently generating a video (video 链很长很贵，一次只处理一篇)
const processingVideo = new Set();
// rowId -> video 失败次数（per process）
const videoFailCounts = new Map();
const MAX_VIDEO_FAILURES = 2; // video 比图更贵，失败上限更保守
// video 生成节流：规划时按每周挑 1 篇打 compose_spec.wants_video 标记；
// worker 侧再加一道安全阀——有未审核视频(video_status='review')时不生成
// 下一篇。即使误标 10 篇也只会"审一篇出下一篇"，不会失控烧钱。

let senders = null; // { sendMessage, sendPhoto, sendImageReviewCard }
let timer = null;
let firstTimer = null;
let ticking = false;
let lastReminderDateMYT = null;

// Account-level API failures (billing cap, quota, rate limit) fail EVERY
// call — treating them per-row burned 3 strikes on each row and spammed a
// failure notice per post. Instead: pause the imagery stages and retry
// automatically after the pause. One notification per process.
const GLOBAL_ERROR_PAUSE_MS = 15 * 60_000;
let pauseImageryUntil = 0;
let pauseNotified = false;

function isGlobalApiError(message) {
  return /billing|hard limit|insufficient_quota|exceeded your current quota|rate limit|too many requests|429/i
    .test(message || '');
}

// ============================================
// review_notes markers written by the Dashboard image review
// ============================================

/**
 * Parse an image-action marker out of review_notes.
 * Returns { scene?: string, productNext?: boolean, recompose?: boolean, raw: boolean }.
 * raw=true when review_notes carried a marker (and should be cleared after use).
 */
function parseImageMarker(reviewNotes) {
  const notes = (reviewNotes || '').trim();
  // 2026-08-06:[img] = 对成品图的任意意见(字太大/颜色不对/风扇位置太高)。
  // [scene] 只能表达"换个背景",而现在整张图(含排版文字)都是模型一次画的 ——
  // 她真正想改的多半根本不是背景。
  if (notes.startsWith('[img]')) {
    return { feedback: notes.slice('[img]'.length).trim(), raw: true };
  }
  // 只组装提示词、不出图 —— 给部署探针用(2026-08-06)。
  // 上一次验部署靠"新功能本身就是探针",但配图这条路上任何探针都要真出一张图。
  // 有了这个之后,验"线上有没有接上反馈"是免费的:发这个 marker,
  // 看 compose_spec.design_prompt 里有没有她那句话。
  if (notes.startsWith('[img-dryrun]')) {
    return { feedback: notes.slice('[img-dryrun]'.length).trim(), promptOnly: true, raw: true };
  }
  if (notes.startsWith('[scene]')) {
    const t = notes.slice('[scene]'.length).trim();
    // 场景描述本身就是一种图上意见,统一走同一条通道往下传
    return { scene: t, feedback: t, raw: true };
  }
  if (notes === '[product-next]') {
    return { productNext: true, raw: true };
  }
  if (notes === '[recompose]') {
    // Dashboard 编辑（改字/换产品/换位置）已直接写入 compose_spec /
    // source_product_image —— 只需重合成，不调图像 AI
    return { recompose: true, raw: true };
  }
  return { raw: false };
}

/** Cycle to the next product image (same logic as the Telegram change-product exit). */
async function cycleProductImage(row) {
  const allImages = listProductImages();
  const currentImage = row.source_product_image || '';
  let nextIndex = 0;
  for (let i = 0; i < allImages.length; i++) {
    if (allImages[i].filename === currentImage) {
      nextIndex = (i + 1) % allImages.length;
      break;
    }
  }
  const nextImage = allImages[nextIndex].filename;
  await writeSourceProductImage(row.id, nextImage);
  return nextImage;
}

// ============================================
// Single-row generation
// ============================================

/**
 * Generate (or regenerate) imagery for one row. Handles markers, failure
 * accounting, and marker cleanup. Pipeline itself moves the row to
 * status='image_ready' on success (legal from both copy_approved and
 * image_retry).
 *
 * @param {object} row - content_calendar row
 * @param {boolean} isRetry - true when consuming an image_retry row (forces fresh regen)
 * @returns {Promise<{success: boolean, imageUrl?: string, error?: string}>}
 */
async function processRow(row, isRetry) {
  if (processing.has(row.id)) {
    return { success: false, error: 'already processing', contention: true };
  }
  processing.add(row.id);
  try {
    // Fresh read — the caller's row may be stale by the time we get here.
    const live = await supabase.getContentCalendar(row.id);
    if (!live) return { success: false, error: 'row disappeared' };
    if (live.image_status === 'generating') {
      // In flight elsewhere (e.g. old container during a deploy overlap)
      return { success: false, error: 'in flight in another process', contention: true };
    }

    const marker = parseImageMarker(live.review_notes);
    let topicOverride = null;
    let current = live;

    if (marker.productNext) {
      await cycleProductImage(live);
      // re-read so the pipeline sees the new source_product_image
      current = await supabase.getContentCalendar(row.id);
      if (!current) return { success: false, error: 'row disappeared' };
    }
    if (marker.scene) {
      // 合成版管线里这是场景提示（sceneHint）：prompt 导出会单独带上 topic，
      // 这里只传审核者要求的场景描述本身
      topicOverride = marker.scene;
    }
    // 2026-08-06:ai_reference（现在除节庆外的**全部**帖子）此前完全不读
    // topicOverride —— 她说"换成傍晚的客厅"，marker 存了、worker 传了，
    // 那条分支根本没接，随机重出一张后还把 marker 当作"已消费"清掉。
    // 界面显示"已重新生成"，她的话一个字都没用上。
    // 这条通道才是真正到达出图提示词的那条。
    const imageFeedback = marker.feedback || null;

    // Cross-process claim: conditional PATCH image_status -> 'generating'
    // guarded on the value we just read. During a Railway deploy overlap two
    // processes can race on the same row; exactly one wins this PATCH, the
    // loser gets a TOCTOU conflict and skips WITHOUT counting a failure.
    try {
      await updateImageRow(row.id, { image_status: 'generating' }, live.image_status);
    } catch (claimErr) {
      return { success: false, error: `claim lost: ${claimErr.message}`, contention: true };
    }

    // 换产品/Dashboard 编辑：复用已有云端背景，纯确定性重合成（无 AI 调用）
    const recomposeOnly = Boolean(marker.productNext || marker.recompose);

    const result = await runImageryPipeline(row.id, {
      topicOverride,
      imageFeedback,
      promptOnly: Boolean(marker.promptOnly),
      fresh: Boolean(isRetry),
      recomposeOnly,
    });

    if (result.success) {
      failCounts.delete(row.id);
      notifiedFailure.delete(row.id);
      if (marker.raw) {
        // marker consumed — clear it so the next retry is a plain regenerate
        await supabase.updateContentCalendar(row.id, { review_notes: null });
      }
      return result;
    }

    // Genuine failure. Release a claim the pipeline may not have cleared
    // (e.g. it failed before scene-gen took over the image_status flow),
    // otherwise the row stays 'generating' and is unclaimable until restart.
    try {
      await updateImageRow(row.id, { image_status: 'failed' }, 'generating');
    } catch (_) {
      // already moved on (scene-gen set failed itself) — fine
    }

    // Account-level failure: pause all imagery, keep the row's strikes
    // intact (it did nothing wrong), notify once.
    if (isGlobalApiError(result.error)) {
      pauseImageryUntil = Date.now() + GLOBAL_ERROR_PAUSE_MS;
      console.error(`[worker] account-level API error — pausing imagery ${GLOBAL_ERROR_PAUSE_MS / 60000} min: ${result.error}`);
      if (!pauseNotified && row.chat_id && senders) {
        pauseNotified = true;
        try {
          await senders.sendMessage(
            row.chat_id,
            `⚠️ Image generation is paused: the image API rejected all requests ` +
            `(${(result.error || '').slice(0, 120)}).\n` +
            `This is usually an OpenAI billing/quota limit. Generation will retry ` +
            `automatically every ${GLOBAL_ERROR_PAUSE_MS / 60000} minutes once the limit is lifted.`
          );
        } catch (notifyErr) {
          console.error('[worker] pause notice send error:', notifyErr.message);
        }
      }
      return { ...result, globalPause: true };
    }

    const fails = (failCounts.get(row.id) || 0) + 1;
    failCounts.set(row.id, fails);
    console.error(`[worker] imagery failed (${fails}/${MAX_FAILURES_PER_ROW}) for row ${row.id}: ${result.error}`);

    if (fails >= MAX_FAILURES_PER_ROW && !notifiedFailure.has(row.id) && row.chat_id && senders) {
      notifiedFailure.add(row.id);
      try {
        await senders.sendMessage(
          row.chat_id,
          `⚠️ Image generation failed ${MAX_FAILURES_PER_ROW} times for "${row.topic || row.id}".\n` +
          `You can retry, upload your own image, or skip the image from the Dashboard.`
        );
      } catch (notifyErr) {
        console.error('[worker] failure notice send error:', notifyErr.message);
      }
    }
    return result;
  } catch (err) {
    console.error(`[worker] processRow uncaught for ${row.id}:`, err.message);
    const fails = (failCounts.get(row.id) || 0) + 1;
    failCounts.set(row.id, fails);
    return { success: false, error: err.message };
  } finally {
    processing.delete(row.id);
  }
}

function isClaimable(row) {
  return (
    !processing.has(row.id) &&
    (failCounts.get(row.id) || 0) < MAX_FAILURES_PER_ROW &&
    row.image_status !== 'generating' // in-flight elsewhere (Telegram inline flow)
  );
}

/**
 * Run fn(item) over items with at most `limit` in flight.
 * Rejections are contained per item (processRow never throws by contract,
 * but a defensive catch keeps one bad item from sinking the pool).
 */
async function runPool(items, limit, fn) {
  const results = new Array(items.length);
  let next = 0;
  async function lane() {
    while (next < items.length) {
      const i = next++;
      try {
        results[i] = await fn(items[i]);
      } catch (err) {
        results[i] = { success: false, error: err.message };
      }
    }
  }
  const lanes = [];
  for (let i = 0; i < Math.min(limit, items.length); i++) lanes.push(lane());
  await Promise.all(lanes);
  return results;
}

// ============================================
// Tick stages
// ============================================

/** Stage 1+3: plans in production — batch imagery, then auto-schedule when done. */
async function processPlansInProduction() {
  const plans = await supabasePlans.listContentPlans({ status: 'in_production' });
  processPlansInProduction._lastPlanIds = new Set(plans.map(p => p.id));
  for (const plan of plans) {
    const rows = await supabase.listContentCalendarByPlanId(plan.id);
    const active = rows.filter(r => r.status !== 'rejected');
    if (active.length === 0) continue;

    const copyQueue = active.filter(r => r.status === 'copy_approved' && isClaimable(r));
    const retryQueue = active.filter(r => r.status === 'image_retry' && isClaimable(r));

    // "Batch started" notice: stateless DB check so a process restart
    // mid-batch does not re-announce — if any row already progressed past
    // the untouched state, the batch was announced before.
    const batchAlreadyStarted =
      active.some(r => ['image_ready', 'image_retry', 'approved', 'published'].includes(r.status)) ||
      active.some(r => r.image_status && r.image_status !== 'pending');

    if (copyQueue.length > 0 && !batchAlreadyStarted && !notifiedBatchStart.has(plan.id) && plan.chat_id && senders) {
      notifiedBatchStart.add(plan.id);
      const etaMin = Math.ceil((copyQueue.length / IMAGE_CONCURRENCY) * 1.5);
      try {
        await senders.sendMessage(
          plan.chat_id,
          `🖼️ Image generation started for ${copyQueue.length} post(s) in the ${plan.month} plan.\n` +
          `Estimated time: about ${etaMin} minute(s). I will let you know when they are ready for review.`
        );
      } catch (err) {
        console.error('[worker] batch start notice error:', err.message);
      }
    }

    let processed = 0;
    // The pause check inside the lane stops remaining queue items as soon
    // as an account-level error is detected mid-batch.
    const guarded = (isRetry) => (row) =>
      Date.now() < pauseImageryUntil
        ? Promise.resolve({ success: false, contention: true, error: 'imagery paused' })
        : processRow(row, isRetry);
    const copyResults = await runPool(copyQueue, IMAGE_CONCURRENCY, guarded(false));
    const retryResults = await runPool(retryQueue, IMAGE_CONCURRENCY, guarded(true));
    for (const r of [...copyResults, ...retryResults]) {
      if (r && r.success) processed++;
    }

    // Re-read to evaluate plan completion
    const fresh = await supabase.listContentCalendarByPlanId(plan.id);
    const freshActive = fresh.filter(r => r.status !== 'rejected');
    if (freshActive.length === 0) continue;

    const pendingImagery = freshActive.filter(r => r.status === 'copy_approved' || r.status === 'image_retry');
    const allDecided = freshActive.every(r => r.status === 'approved' || r.status === 'published');
    const anyApproved = freshActive.some(r => r.status === 'approved');

    if (processed > 0 && pendingImagery.length === 0 && !allDecided && plan.chat_id && senders) {
      // Batch just finished this tick; images await human review.
      // 带上降级警告 —— 2026-07-30 事故:两张成品"没有风扇"却安静通过,
      // 因为降级只写在 console。缺要素必须出现在人看得到的地方。
      const flagged = fresh
        .map((r) => {
          const spec = readSpec(r);
          return spec.warnings && spec.warnings.length
            ? `• ${(r.topic || r.id).slice(0, 40)}: ${spec.warnings.join('; ')}`
            : null;
        })
        .filter(Boolean);
      try {
        await senders.sendMessage(
          plan.chat_id,
          `✅ Imagery is ready for the ${plan.month} plan.\n` +
          `Please review the images on the Dashboard (Marketing → Image Review).` +
          (flagged.length
            ? `\n\n⚠️ ${flagged.length} post(s) generated with missing elements — check these first:\n${flagged.join('\n')}`
            : '')
        );
      } catch (err) {
        console.error('[worker] batch done notice error:', err.message);
      }
    }

    if (allDecided && !anyApproved) {
      // Every active row is already published — nothing left to schedule.
      // Close the plan out so it stops being re-processed every tick.
      try {
        await supabasePlans.updateContentPlan(plan.id, { status: 'completed' });
        console.log(`[worker] plan ${plan.id} fully published — marked completed`);
      } catch (err) {
        console.error(`[worker] failed to complete plan ${plan.id}:`, err.message);
      }
      continue;
    }

    if (allDecided) {
      // M-6: every active post passed image review — auto-schedule
      try {
        const scheduledRows = await schedulePlan(plan.id); // also sets plan status='scheduled'
        console.log(`[worker] auto-scheduled plan ${plan.id}: ${scheduledRows.length} rows`);
        if (plan.chat_id && senders) {
          const table = formatScheduleTable(scheduledRows);
          await senders.sendMessage(plan.chat_id, table, { parse_mode: 'Markdown' });
          await senders.sendMessage(
            plan.chat_id,
            `📅 All posts scheduled. I will remind you on each publish day at ${REMINDER_HOUR_MYT}:00 (MYT).`
          );
        }
      } catch (err) {
        console.error(`[worker] auto-schedule failed for plan ${plan.id}:`, err.message);
      }
    }
  }
}

/** Stage 2: orphan image_retry rows outside in_production plans (incl. single-post flow). */
async function processOrphanRetries(plansInProduction) {
  const rows = await supabase.listContentCalendar({ status: 'image_retry' });
  if (!plansInProduction) {
    plansInProduction = new Set(
      (await supabasePlans.listContentPlans({ status: 'in_production' })).map(p => p.id)
    );
  }
  const queue = rows.filter(row =>
    !(row.plan_id && plansInProduction.has(row.plan_id)) && isClaimable(row)); // plan rows handled by stage 1
  await runPool(queue, IMAGE_CONCURRENCY, async (row) => {
    if (Date.now() < pauseImageryUntil) {
      return { success: false, contention: true, error: 'imagery paused' };
    }
    const result = await processRow(row, true);
    if (result.success && !row.plan_id && row.chat_id && senders && senders.sendImageReviewCard) {
      // Single-post flow reviews in Telegram — send the review card there.
      // Plan rows review on the Dashboard, no Telegram card needed.
      try {
        await senders.sendImageReviewCard(row.chat_id, row.id, result.imageUrl || '(scene)', 'regenerated', result.isDryRun);
      } catch (err) {
        console.error('[worker] review card send error:', err.message);
      }
    }
    return result;
  });
}

// ============================================
// Stage 3.5: 每周动态视频（方案 B）
// ============================================

function readSpec(row) {
  const raw = row && row.compose_spec;
  if (!raw) return {};
  if (typeof raw === 'object') return raw;
  try { return JSON.parse(raw); } catch (_) { return {}; }
}

/**
 * 生成动态视频队列。
 * 候选：status ∈ {image_ready, approved}、compose_spec.wants_video===true、
 *       尚无 video_url、未在生成中、失败未超上限。
 * 安全阀：全局已有 video_status==='review'（等人工审核）时，本轮不生成新的——
 *         一次只推进一篇，天然节流 + 防误标失控。每 tick 最多处理 1 篇。
 */
async function processVideoQueue() {
  if (Date.now() < pauseImageryUntil) return; // 沿用图像账户级暂停闸

  let candidates = [];
  try {
    const rows = [];
    for (const status of ['image_ready', 'approved']) {
      rows.push(...(await supabase.listContentCalendar({ status })));
    }
    candidates = rows.filter((r) => {
      const spec = readSpec(r);
      return spec.wants_video === true && !spec.video_url;
    });
    // 安全阀：有未审核视频在等 → 本轮不生成新视频
    const hasOutstanding = rows.some((r) => readSpec(r).video_status === 'review');
    if (hasOutstanding) return;
  } catch (err) {
    console.error('[worker] video queue list failed:', err.message);
    return;
  }

  const next = candidates.find((r) =>
    !processingVideo.has(r.id) && (videoFailCounts.get(r.id) || 0) < MAX_VIDEO_FAILURES
  );
  if (!next) return;

  processingVideo.add(next.id);
  try {
    console.log(`[worker] generating weekly video for row ${next.id} ("${next.topic || ''}")`);
    const result = await generatePostVideo(next, { log: (m) => console.log(`  [video] ${m}`) });

    if (result.dryRun) {
      console.log('[worker] video dry-run (keys missing) — skipping');
      return;
    }
    if (!result.success) {
      // 账户级错误：沿用图像暂停闸，不计入本行 strike
      if (isGlobalApiError(result.error)) {
        pauseImageryUntil = Date.now() + GLOBAL_ERROR_PAUSE_MS;
        console.error(`[worker] video hit account-level API error — pausing: ${result.error}`);
        return;
      }
      const fails = (videoFailCounts.get(next.id) || 0) + 1;
      videoFailCounts.set(next.id, fails);
      console.error(`[worker] video failed (${fails}/${MAX_VIDEO_FAILURES}) for ${next.id}: ${result.error}`);
      if (fails >= MAX_VIDEO_FAILURES && next.chat_id && senders) {
        try {
          await senders.sendMessage(next.chat_id,
            `⚠️ Weekly video generation failed for "${next.topic || next.id}". It will not retry automatically. ` +
            `You can un-flag and re-flag it later to try again.`);
        } catch (_) {}
      }
      return;
    }

    // 成功：video_url 写入 compose_spec，标记待审
    const spec = readSpec(next);
    spec.video_url = result.videoUrl;
    spec.video_status = 'review';
    try {
      await supabase.updateContentCalendar(next.id, { compose_spec: spec });
    } catch (err) {
      console.error(`[worker] failed to persist video_url for ${next.id}:`, err.message);
    }
    videoFailCounts.delete(next.id);
    console.log(`[worker] weekly video ready for ${next.id}: ${result.videoUrl}`);

    if (next.chat_id && senders && senders.sendVideoReviewCard) {
      try {
        await senders.sendVideoReviewCard(next.chat_id, next.id, result.videoUrl, next.topic || '');
      } catch (err) {
        console.error('[worker] video review card send error:', err.message);
      }
    }
  } catch (err) {
    console.error(`[worker] processVideoQueue uncaught for ${next.id}:`, err.message);
  } finally {
    processingVideo.delete(next.id);
  }
}

/** Stage 4: daily publish reminder (M-7). */
async function runDailyReminder() {
  if (!senders) return;
  const todayMYT = getMalaysiaDateStr();
  const nowUTC = new Date();
  const hourMYT = (nowUTC.getUTCHours() + 8) % 24;
  if (hourMYT < REMINDER_HOUR_MYT) return;
  if (lastReminderDateMYT === todayMYT) return;
  lastReminderDateMYT = todayMYT; // set before running; publish_reminder_sent makes re-runs idempotent anyway
  try {
    const result = await checkTodayPosts(senders.sendMessage, senders.sendPhoto, null);
    console.log(`[worker] daily reminder: ${result.sent} sent, ${result.failed} failed`);
  } catch (err) {
    console.error('[worker] daily reminder error:', err.message);
  }

  // ── 节日表快用完提醒(Edwin 的要求:不要靠他记得)──
  // 每周一发一次,发给最近在用 bot 的那个 chat(最新 content_calendar 行的 chat_id)。
  try {
    const warn = require('./festival-calendar').coverageWarning(todayMYT);
    const isMondayMYT = new Date(todayMYT + 'T00:00:00Z').getUTCDay() === 1;
    if (warn && isMondayMYT) {
      const rows = await supabase.listContentCalendar({ limit: 5 });
      const target = (rows.find((r) => r.chat_id) || {}).chat_id;
      if (target) { await senders.sendMessage(target, warn); console.log('[worker] festival coverage warning sent'); }
      else console.warn('[worker] festival coverage warning pending: no target chat');
    }
  } catch (err) {
    console.error('[worker] festival coverage check error:', err.message);
  }
}

// ============================================
// Crash recovery
// ============================================

/**
 * Reset rows stuck in image_status='generating' from a previous process.
 * Only one bot process exists, so at startup nothing can legitimately be
 * generating. generating -> failed is a legal image transition.
 */
async function recoverStuckRows() {
  try {
    const stuck = [];
    for (const status of ['copy_approved', 'image_retry']) {
      const rows = await supabase.listContentCalendar({ status });
      stuck.push(...rows.filter(r => r.image_status === 'generating'));
    }
    for (const row of stuck) {
      try {
        // Deliberately bypasses image-state.transitionImageStatus: this is
        // crash recovery, and generating -> failed is legal anyway.
        await supabase.updateContentCalendar(row.id, { image_status: 'failed' });
        console.log(`[worker] recovered stuck row ${row.id} (generating -> failed)`);
      } catch (err) {
        console.error(`[worker] stuck-row recovery failed for ${row.id}:`, err.message);
      }
    }
  } catch (err) {
    console.error('[worker] recoverStuckRows error:', err.message);
  }
}

// ============================================
// Loop
// ============================================

// ============================================
// 文案驳回重写队列（2026-08-06）
// ============================================
//
// 缺口:Dashboard 上「Request Changes」把意见存进 review_notes 就没了下文 ——
// Dashboard 里没有任何按钮能触发重写,帖子停在 copy_done,而 start-imagery 要求
// "没有 copy_done 剩下"才能开工,于是**驳回一篇就把整月卡死**,她还不知道为什么。
//
// 为什么走 worker 而不是让 Dashboard 自己生成:Dashboard 是独立服务,在那边
// 重实现一份生成逻辑 = 把选品池/角度配额/按排期日/编造拦截这四样再漏一次
// (那正是 8/5 收口掉的东西)。所以沿用配图那条路已经验证过的解耦方式:
// **Dashboard 只写 DB 状态,worker 认领后走同一个 generateCopy**。
//
// 状态用 'rejected':状态机本来就有 pending_review→rejected→copy_done 这条边,
// 而且 start-imagery 已经把 rejected 排除在 active 之外 —— 于是"驳回不再卡住整月"
// 是免费拿到的,不需要新状态、不需要改 DB 的 CHECK 约束。
const rewritingCopy = new Set();

async function processCopyRewrites() {
  if (!senders || typeof senders.rewriteCopy !== 'function') return;
  let rows;
  try {
    rows = await supabase.listContentCalendar({ status: 'rejected' });
  } catch (err) {
    console.error('[worker] 读取驳回队列失败:', err.message);
    return;
  }
  const notes = require('./review-notes');
  for (const row of rows || []) {
    if (rewritingCopy.has(row.id)) continue;
    const raw = (row.review_notes || '').trim();
    // 她按了"跳过这篇" → 保持 rejected,不重写(整月照常往下走)
    if (/^\[skip\]/i.test(raw)) continue;
    // 没写意见就驳回 = "再来一版",也走重写
    rewritingCopy.add(row.id);
    try {
      if (notes.atLimit(raw)) {
        // 改了 5 次还不满意 —— 再改下去说明方向根本不对,不该继续烧
        if (!notifiedRewriteLimit.has(row.id) && row.chat_id) {
          notifiedRewriteLimit.add(row.id);
          await senders.sendMessage(row.chat_id,
            `「${row.topic}」已经改了 ${notes.noteCount(raw)} 次还是不满意。\n` +
            `再改下去多半不是措辞问题,是方向不对 —— 建议整篇重来(换角度/换产品),` +
            `或者先跳过这篇。\n\n之前提过的意见:\n` +
            notes.parseNotes(raw).map((n) => `  ${n.n}. ${n.text}`).join('\n'));
        }
        continue;
      }
      const result = await senders.rewriteCopy(row);
      if (result && result.ok) {
        console.log(`[worker] 带意见重写完成: ${row.topic} (第 ${notes.noteCount(raw)} 条意见)`);
      }
    } catch (err) {
      console.error(`[worker] 重写失败 ${row.id}:`, err.message);
      // 失败就留在 rejected,下一轮再试;连续失败由 rewriteCopy 侧决定是否通知
    } finally {
      rewritingCopy.delete(row.id);
    }
  }
}

// 已经提示过"改太多次"的行(每进程一次,别每 45 秒刷一条)
const notifiedRewriteLimit = new Set();

async function tick() {
  if (ticking) return; // previous tick still running (image gen can exceed TICK_MS)
  ticking = true;
  try {
    // 文案重写排在出图前面:她驳回的东西越早回到她面前越好,而且它便宜(纯文本)
    await processCopyRewrites();
    if (Date.now() >= pauseImageryUntil) {
      await processPlansInProduction();
      await processOrphanRetries(processPlansInProduction._lastPlanIds);
      // 动态视频排在静态图之后：图先出齐（含 spec.image_texts），视频复用其文案
      await processVideoQueue();
    } else {
      console.log(`[worker] imagery paused (account-level API error), resumes in ${Math.ceil((pauseImageryUntil - Date.now()) / 60000)} min`);
    }
    await runDailyReminder();
  } catch (err) {
    console.error('[worker] tick error:', err.message);
  } finally {
    ticking = false;
  }
}

/**
 * Start the worker.
 * @param {object} sendersArg
 * @param {Function} sendersArg.sendMessage - (chatId, text, opts) => Promise
 * @param {Function} sendersArg.sendPhoto - (chatId, urlOrBuffer, opts) => Promise
 * @param {Function} [sendersArg.sendImageReviewCard] - Telegram review card for single-flow rows
 */
function start(sendersArg) {
  if (timer) return; // already started
  senders = sendersArg;
  if (!supabase.isConfigured()) {
    console.warn('[worker] Supabase not configured — worker disabled');
    return;
  }
  recoverStuckRows().catch(() => {});
  // 字体自检：容器缺字体时 sharp 静默输出空白文字（July 批次线上无字的根因）
  require('./compose').canRenderText().then((ok) => {
    if (!ok) {
      console.error('[worker] FONT CHECK FAILED — SVG text renders blank. ' +
        'Install fontconfig + fonts-dejavu-core in the container (see Dockerfile). ' +
        'All composed images will be missing their text until fixed.');
    } else {
      console.log('[worker] font check passed — text rendering available');
    }
  }).catch(() => {});
  firstTimer = setTimeout(tick, FIRST_TICK_DELAY_MS);
  timer = setInterval(tick, TICK_MS);
  console.log(`[worker] started (tick=${TICK_MS / 1000}s, daily reminder at ${REMINDER_HOUR_MYT}:00 MYT)`);
}

function stop() {
  if (timer) clearInterval(timer);
  if (firstTimer) clearTimeout(firstTimer);
  timer = null;
  firstTimer = null;
}

// ============================================
// 视频标记 / 审核辅助（供 index.js Telegram 回调 & Dashboard 调用）
// ============================================

/** 规划/Dashboard 时给某篇打上"做视频"标记（compose_spec.wants_video=true）。 */
async function markWantsVideo(rowId, wants = true) {
  const row = await supabase.getContentCalendar(rowId);
  if (!row) throw new Error('row not found');
  const spec = readSpec(row);
  spec.wants_video = Boolean(wants);
  await supabase.updateContentCalendar(rowId, { compose_spec: spec });
  return spec;
}

/** 人工审核结果：approve → video_status='approved'；reject → 清掉视频、允许重来。 */
async function resolveVideoReview(rowId, approved) {
  const row = await supabase.getContentCalendar(rowId);
  if (!row) throw new Error('row not found');
  const spec = readSpec(row);
  if (approved) {
    spec.video_status = 'approved';
  } else {
    // 驳回：清掉 video_url，wants_video 保持 true → 下一轮重新生成一版
    delete spec.video_url;
    spec.video_status = 'rejected';
    videoFailCounts.delete(rowId);
  }
  await supabase.updateContentCalendar(rowId, { compose_spec: spec });
  return spec;
}

module.exports = {
  start,
  stop,
  tick, // exposed for tests and manual triggering
  processing,
  processRow, // unified regeneration entry — index.js inline retry routes through this
  parseImageMarker,
  processCopyRewrites,
  markWantsVideo,
  resolveVideoReview,
  // internals exposed for tests
  _internal: {
    processRow,
    processPlansInProduction,
    processOrphanRetries,
    processVideoQueue,
    runDailyReminder,
    recoverStuckRows,
    isClaimable,
    failCounts,
    notifiedBatchStart,
    videoFailCounts,
    processingVideo,
    setLastReminderDate: (d) => { lastReminderDateMYT = d; },
    setSenders: (s) => { senders = s; }, // inject senders without starting timers (tests)
    runPool,
  },
};
