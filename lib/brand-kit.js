...
const PRODUCT_SLOTS = {
  ...
  // hero_top：贴近顶边、几乎撑满上半幅（product_intro 等图形背景模板用）——
  // 参考图里的风扇不是"漂浮在中央的小图标"，是顶着画布上缘的主视觉。
  hero_top:     { cx: 0.50, cy: 0.15, w: 0.96, h: 0.50 },
};
...
function buildTextPresets(titleSlot, colors, fontFamily) {
  ...
  return {
    // 全部 backgroundOpacity: 0 —— 14 张真实 reference 里没有一张用黑底胶囊
    // 压文字，都是裸字+描边直接压在背景上（2026-07-24 真实效果图评审实证：
    // 半透明黑框在渐变/图形背景上显脏，像补丁）。对比度交给描边处理。
    title: {
