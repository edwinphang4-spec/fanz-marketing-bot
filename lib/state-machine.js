...
const TRANSITIONS = {
  ...
  // 2026-08-06:补上 copy_done → rejected。
  // Dashboard 的「Request Changes」就是从 copy_done 驳回的,而 Dashboard 直连
  // supabase-js、不走这个状态机 —— 于是出现"线上跑得通、但状态机说它非法"。
  // 装部署探针时被这条挡住才发现:同样的转移,bot 自己反而做不了。
  // 状态机是转移合法性的真源,真源和实际行为对不上,迟早在别的路径上炸。
